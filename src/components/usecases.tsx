		...
		<section className="py-20 px-6 bg-gradient-to-b from-indigo-50 to-white">
			<div className="mx-auto max-w-6xl">
				<h2 className="text-4xl font-extrabold text-center text-indigo-700 mb-12">
					Event Organiser Types
				</h2>
				<div className="grid gap-14 md:grid-cols-3">
					{organiserTypes.map((type) => (
						<div
							key={type.title}
							className="flex flex-col items-center gap-4 p-8 bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300"
						>
							<Image
								src={type.icon}
								alt={`${type.title} icon`}
				...
				<h2 className="text-4xl font-extrabold text-center text-indigo-700 mt-20 mb-12">
					Pricing Plans
				</h2>
				<div className="grid gap-14 md:grid-cols-3">
					{pricingPlans.map((plan) => (
						<div
							key={plan.tier}
							className="flex flex-col items-center gap-4 p-8 bg-indigo-50 border border-indigo-200 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300"
						>
							<h3 className="text-2xl font-bold text-indigo-600">
								{plan.tier}
							</h3>
